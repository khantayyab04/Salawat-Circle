-- MVP12 correction: streaks are lifetime metrics, not chart-range metrics.
--
-- `get_progress_series` is called for a chosen chart period. Its first
-- implementation also derived active and longest streaks from that period,
-- which truncated an otherwise continuous streak at Monday or the first day
-- of a month. The chart and its period totals remain bounded; only streaks
-- read the caller's full entry history through today.
...
begin
  ...
  history_active_days as (
    select entry.entry_date, sum(entry.amount)::bigint as total
    from public.salawat_entries entry
    where entry.user_id = v_user_id
  ...
  ordered_days as (
    select
      history_active_days.entry_date,
      history_active_days.total,
      history_active_days.entry_date
        - (row_number() over (
          order by history_active_days.entry_date
        ))::integer as streak_group
    from history_active_days
  ),
  streaks as (
    select
  ...
  select
    ...
    coalesce((
      ...
      from streaks
      where streaks.last_day = v_today
      limit 1
    ), 0),
