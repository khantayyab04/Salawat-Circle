...
- **Einladungstoken-Geheimhaltung:** 10-stellige Einladungscodes werden serverseitig
  ausschließlich als SHA-256-Hash (`group_invite_hashes`) gespeichert. Roh-Tokens
  existieren flüchtig auf dem Client des Erstellers oder des Beitretenden und werden
...
- **Brute-Force- & Enumerationsschutz:** Das Erstellen, Vorschauen und Einlösen von
  Einladungscodes sowie die Gruppenerstellung sind über `check_rate_limit` in
...
- **Neutrale Fehlermeldungen (Oracle-Vermeidung):** Ungültige, abgelaufene, verbrauchte
  ...
  (`INVALID_INVITE` / `joinInvalidInviteMessage`). Ein Angreifer kann nicht differenzieren,
  ob ein Token existierte, abgelaufen war oder erschöpft ist.
- **Transaktions-Sperren & Kapazitätsschutz:** Die Einlösungs-RPC (`join_group_by_invite`)
...
- **RLS & Nicht-Mitglieder-Zugriffsschutz:** Direkter Datenbankzugriff auf Gruppentabellen
  ist blockiert. Nicht-Mitglieder besitzen keinerlei Lese- oder Schreibrechte auf Gruppen,
...
- **Nicht-rückwirkende Anonymisierung:** Die Gruppenanonymisierung (Pseudonyme wie `Teilnehmer #A1B2`)
...
- **Zufällige Epochen & Opage Zeilen-IDs:** Alias-Schlüssel (`alias_key`, `alias_epoch`)
...
- **Zwischenablage & Teilen (Nutzerverantwortung):** Das Teilen von Einladungslinks
  oder Codes über das Betriebssystem-Share-Sheet oder die Zwischenablage (`expo-clipboard`)
  überträgt die Geheimhaltung des Tokens in die Verantwortung der Nutzer. Einladungen
